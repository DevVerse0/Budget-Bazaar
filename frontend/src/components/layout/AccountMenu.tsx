...
export default function AccountMenu(){
  ...
  return (<div className="relative">
    ...
    {open && <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-2xl border overflow-hidden z-50" onMouseLeave={()=>setOpen(false)}>
      <Link href="/account" className="block px-4 py-2 text-sm hover:bg-gray-50">My Account</Link>
      <Link href="/account" className="block px-4 py-2 text-sm hover:bg-gray-50">My Orders</Link>
      <Link href="/wishlist" className="block px-4 py-2 text-sm hover:bg-gray-50">Wishlist</Link>
      <Link href="/account" className="block px-4 py-2 text-sm hover:bg-gray-50">Account Settings</Link>
      <button onClick={async()=>{ await supabase.auth.signOut(); localStorage.removeItem('token'); location.href='/login'; }} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-50">Logout</button>
    </div>}
  </div>);
}
