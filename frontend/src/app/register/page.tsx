  ...
  return (<div className="min-h-[80vh] flex items-center justify-center bg-gradient-to-br from-navy via-[#0f1a30] to-navy p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 border">
      ...
      <form onSubmit={submit} className="space-y-3">
        <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">👤</span><input value={name} onChange={e=>setName(e.target.value)} placeholder="Rahat Islam" className="w-full border rounded-xl pl-9 pr-3 py-3 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-gold focus:border-gold outline-none transition" required/></div>
        <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">📱</span><input value={mobile} onChange={e=>setMobile(e.target.value)} placeholder="01712 345 678" className="w-full border rounded-xl pl-9 pr-3 py-3 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-gold focus:border-gold outline-none transition"/></div>
        <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">✉</span><input value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@gmail.com" type="email" className="w-full border rounded-xl pl-9 pr-3 py-3 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-gold focus:border-gold outline-none transition" required/></div>
        <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">🔒</span><input value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min. 6 characters" type="password" className="w-full border rounded-xl pl-9 pr-3 py-3 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-gold focus:border-gold outline-none transition" required/></div>
